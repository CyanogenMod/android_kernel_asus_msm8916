...
//<asus-danielchan20150323>>>>>>>>>+
#include <linux/kernel.h>
//<ASUS-danielchan20150323><<<<<<<<+ 

//<ASUS-annacheng20150129>>>>>>>>>>>>
#define Driverversion  "1.0.0"
#define VENDOR  "bosch-bmi160"
struct proc_dir_entry *bmi160sensor_entry=NULL;
//<ASUS-annacheng20150129><<<<<<<<<<<<+ 
...
//<asus-danielchan20150323>>>>>>>>>+
unsigned int t_acc_x;
unsigned int t_acc_y;
unsigned int t_acc_z;
//<ASUS-danielchan20150323><<<<<<<<+ 
...
//<asus-annacheng20150129>>>>>>>>>>>>>>>>>>>>>>>+
struct bmi_client_data *client_data_zx;

static int bmi160sensor_proc_show(struct seq_file *m, void *v) {
	
		int ret = 0,retval=0;		
		struct bmi_client_data *client_data = client_data_zx;
		
		retval = bmi_check_chip_id(client_data);
		if (retval < 0)
			printk(KERN_INFO "[bmi160] i2c err\n");
		...
		if(Driverversion != NULL){
	  	...
	  	if(client_data->chip_id){//who am i
	  		ret =seq_printf(m," Vendor:%s(0x%x)\n", VENDOR,client_data->chip_id);
		}else{
			ret =seq_printf(m," Vendor:%s(0x%x)\n", VENDOR,client_data->chip_id);
		}

		retval = bmi_check_chip_id(client_data);
		printk("anna-sensor bmi_check_chip_id : 0x00=0x%x \n", retval);
...
static int bmi160sensor_proc_open(struct inode *inode, struct  file *file) {
  return single_open(file, bmi160sensor_proc_show, NULL);
}

static const struct file_operations bmi160sensor_proc_fops = {
  .owner = THIS_MODULE,
  .open = bmi160sensor_proc_open,
  .read = seq_read,
  .llseek = seq_lseek,
  .release = single_release,
};

int create_asusproc_bmi160sensor_status_entry(void){
	bmi160sensor_entry = proc_create("gsensor_status", S_IWUGO| S_IRUGO, NULL,&bmi160sensor_proc_fops);
 	if (!bmi160sensor_entry)
       		 return -ENOMEM;
...
//<asus-danielchan20150323>>>>>>>>>+
			len = sprintf(buf, "%s %d %d %d %llu\n",
					ACC_FIFO_HEAD,
					acc_frame_arr[i].x*t_acc_x,
					acc_frame_arr[i].y*t_acc_y,
					acc_frame_arr[i].z*t_acc_z,
					current_frm_ts);
//<ASUS-danielchan20150323><<<<<<<<+ 
...
//<asus-danielchan20150323>>>>>>>>>+
				len = sprintf(buf,
			"%s %d %d %d %llu %s %d %d %d %llu %s %d %d %d %llu ",
					GYRO_FIFO_HEAD,
					gyro_frame_arr[i].x,
					gyro_frame_arr[i].y,
					gyro_frame_arr[i].z,
...
//<ASUS-danielchan20150323><<<<<<<<+ 
	...
	//<asus-annacheng20150129>>>>>>>>>>>>>>>>>>>>>>>+
	if(create_asusproc_bmi160sensor_status_entry())
		printk("[%s] : ERROR to create bmi160sensor proc entry\n",__func__);
	//<asus-annacheng20150129><<<<<<<<<<<<<<<<<+ 
